...
@router.get('/me/profile', dependencies=[Depends(auth_dependency_middleware())])
async def fetch_current_user_profile(request: Request, db:Session=Depends(db.get_db)):
