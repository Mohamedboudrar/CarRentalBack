...
async def confirm_booking(booking_id: str, db:Session=Depends(db.get_db), credentials: HTTPAuthorizationCredentials = Security(security)):
    token = credentials.credentials
    ...
    if vehicle_detail.user_id != current_user.id:
...
async def cancel_booking(booking_id: str, db:Session=Depends(db.get_db), credentials: HTTPAuthorizationCredentials = Security(security)):
    token = credentials.credentials
    current_user = jwt_utils.verify_token_access(token)
    booking_detail = db.query(models.Booking).filter(models.Booking.id == booking_id).first()
    vehicle_detail = db.query(models.Vehicle).filter(models.Vehicle.id == booking_detail.vehicle_id).first()

    if vehicle_detail.user_id != current_user.id or current_user.id != booking_detail.user_id:
